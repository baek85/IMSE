...
class ViDA(nn.Module):
    ...
    def forward(self, x, no_adapt=False):
        ...
        for _ in range(self.steps):
            outputs, loss_dict = self.forward_and_adapt(x, self.model, self.optimizer)
    ...
    def forward_and_adapt(self, x, model, optimizer):
        ...
        loss_dict['uncertainty'] = uncertainty.item()
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
        # Teacher update
...
def configure_model(model, cfg):
    ...
    if cfg.TEST.ckpt!=None:
        ...
        # # module.blocks.1.attn.proj.weight
        # checkpoint1 = torch.load('./checkpoint/imagenet_vit_vida.pt')

        # # module.model.blocks.1.attn.proj.weight
        # checkpoint2 = torch.load('./checkpoint/imagenet_vit_vida_normalizer.pt')
        # # compare checkpoint and checkpoint2
        # for key in checkpoint1.keys():
        #     if key in checkpoint2.keys():
        #         if checkpoint1[key].shape != checkpoint2[key].shape:
        #             print(f"Shape mismatch for {key}: {checkpoint1[key].shape} vs {checkpoint2[key].shape}")
        #         elif torch.all(checkpoint1[key] == checkpoint2[key]):
        #             print(f"{key} is the same in both checkpoints")
        #     else:
        #         print(f"{key} not found in checkpoint2")

        # model.load_state_dict(checkpoint, strict=True)

        for key in model.state_dict().keys():
            replace_key = key.replace('module.model', 'module')
            if key in checkpoint.keys():
                if model.state_dict()[key].shape != checkpoint[key].shape:
                    print(f"Shape mismatch for {key}: {model.state_dict()[key].shape} vs {checkpoint[key].shape}")
                else:
                    model.state_dict()[key].copy_(checkpoint[key])
            elif replace_key in checkpoint.keys():
                if model.state_dict()[key].shape != checkpoint[replace_key].shape:
                    print(f"Shape mismatch for {key}: {model.state_dict()[key].shape} vs {checkpoint[replace_key].shape}")
                else:
                    model.state_dict()[key].copy_(checkpoint[replace_key])
            else:
                print(f"{key} not found in checkpoint")
        # breakpoint()
        # model.load_state_dict(checkpoint, strict=False)
        # torch.save(model.state_dict(), './checkpoint/imagenet_vit_vida_normalizer.pt')      
