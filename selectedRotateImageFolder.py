...
te_transforms_no_normalize = transforms.Compose([transforms.Resize(256),
									transforms.CenterCrop(224),
									transforms.ToTensor()])
...
def prepare_test_data(data_dir='/mnt/nfs_shared_data/dataset/ILSVRC2012', use_transforms=True, batch_size=64, if_shuffle=False, num_workers=0):
    # te_transforms_local = te_transforms if use_transforms else None
    te_transforms_local = te_transforms_no_normalize if use_transforms else None

    validdir = os.path.join(data_dir, 'val')
    teset = SelectedRotateImageFolder(validdir, te_transforms_local, original=False, rotation=False,
                                                rotation_transform=rotation_te_transforms)
    teloader = torch.utils.data.DataLoader(teset, batch_size=batch_size, shuffle=if_shuffle, 
