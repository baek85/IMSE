...
def load_imagenetc(
    ...
    # indices가 제공되면 전체 데이터셋에서 선택해야 하므로 항상 image_list_file 사용
    # (5000개만 있지만, indices는 50000개 범위일 수 있으므로 폴더 스캔)
    if n_examples == 5000 and indices is None:
        imagenet = CustomImageFolder(data_folder_path, transforms_test, 
                                     image_list_file='robustbench/data/imagenet_test_image_ids.txt', 
                                     class_map_file='robustbench/data/imagenet_class_to_id_map.json',
                                     indices=indices)
    else:
        # indices를 사용하거나 n_examples != 5000이면 폴더 전체를 스캔하여 50000개 로딩
        imagenet = CustomImageFolder(data_folder_path, transforms_test, indices=indices)

    # indices가 제공되면 해당 개수만 로딩하므로 배치 크기 조정
    batch_size = len(indices) if indices is not None else n_examples
    test_loader = data.DataLoader(imagenet, batch_size=batch_size,
