...
def load_model(model_name: str,
    ...
    if not isinstance(models[model_name]['gdrive_id'], list):
        ...
        if dataset_ == BenchmarkDataset.imagenet and ('Standard' in model_name) or ('vida' in model_name) or ('ours' in model_name):
            return model.eval()
    ...
    with torch.no_grad():
        for counter in range(n_batches):
            ...
            if output is None:
                ...
                if len(output) == 2:
                    output, loss_dict = output
                    acc += (output.max(1)[1] == y_curr).float().sum()
                    total_samples += output.shape[0]
                    loss_dict['acc'] = acc / total_samples
                    if not debug:
                        wandb.log(loss_dict)
                else:
                    acc += (output.max(1)[1] == y_curr).float().sum()
...
def clean_accuracy_efficiency(model: nn.Module,
                   x: torch.Tensor,
                   y: torch.Tensor,
                   batch_size: int = 100,
                   device: torch.device = None,
    ...
    with torch.no_grad():
        for counter in range(n_batches):
            ...
            # output = model(x_curr)
            start_event.record()
            # flops = FlopCountAnalysis(model, x_curr)
            output = model(x_curr)
            peak_memory = torch.cuda.max_memory_allocated()
            gpeak_memory = peak_memory / 1024 / 1024
            ...
            if output is None:
                ...
                if len(output) == 2:
                    ...
                    total_samples += output.shape[0]
                    loss_dict['acc'] = acc / total_samples
                    # loss_dict['flops'] = total_flops
                    # loss_dict['gflops'] = gflops
                    loss_dict['batch_time'] = total_time_sec
                    loss_dict['peak_memory'] = gpeak_memory
                    if not debug:
                        wandb.log(loss_dict)
                else:
                    acc += (output.max(1)[1] == y_curr).float().sum()
            ...
            if filename is not None and counter == 5:
...
def clean_accuracy_no_adapt(model: nn.Module,
                   x: torch.Tensor,
                   y: torch.Tensor,
                   batch_size: int = 100,
                   device: torch.device = None,
    ...
    with torch.no_grad():
        for counter in range(n_batches):
            ...
            output = model(x_curr, no_adapt=True)
